...
REM REAL OUTPUT
csmith --no-argc --no-bitfields --no-comma-operators --no-consts --no-longlong --no-float --no-math64 --no-safe-math --no-packed-struct --no-volatiles --no-volatile-pointers --no-const-pointers --max-funcs 1 > test.c
